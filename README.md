...
9. 实现了正常应用与 CDN 流量中转（基于 WebSocket/HttpUpgrade/gRPC over TLS） 同时使用。
10. 实现了除 V2Ray/Xray 的 mKCP 应用及 Hysteria 应用（若 NaiveProxy 已启用 HTTP/3 代理应用）之外，其它应用对外都使用 443 端口，各应用互不影响。
11. 实现了除 V2Ray/Xray 的 mKCP 应用及 Hysteria 应用（若 NaiveProxy 已启用 HTTP/3 代理应用）之外，其它应用都支持流量伪装与防探测，且提供流量伪装与防探测的回落或代理网站都支持 HTTP 自动跳转到 HTTPS，SSL/TLS 安全评估报告为 A+（非 AES 算法的密码套件配置示例除外） 等，即所有特征完全与真实网站一致。
...
8. 当前不推荐使用 WebSocket 传输方式的应用直接科学上网（套 CDN 使用无碍）；其相关 Xray 应用推荐使用 HttpUpgrade 传输方式替代，功能相同且效率更高。
9. 若 CDN 流量中转（基于 WebSocket/HttpUpgrade/gRPC over TLS）使用不可信的 CDN 进行中转，V2Ray/Xray 示例推荐使用自身带加密的 VMess 或 Shadowsocks 协议配置；否则推荐使用自身不带加密的 VLESS 或 Trojan 协议配置。
10. 针对 V2Ray 特性，当前服务端配置推荐使用非 AES 算法密码套件配置示例，客户端开启 uTLS 指纹。 
11. 针对 Xray 特性，当前服务端配置依次推荐使用 REALITY Vision 配置示例、XTLS Vision 配置示例、非 AES 算法的密码套件配置示例、REALITY 配置示例，客户端开启 uTLS 指纹。
12. 流量伪装与防探测网站可由其它 WEB 应用软件实现，其支持反代应用（反代 WebSocket、HttpUpgrade、gRPC 及 H2C）与支持回落应用（提供 H2C server 及 HTTP/1.1 server 的 WEB 服务）等取决于自身，其配置自行参考 Caddy 或 Nginx 对应配置示例。
13. 附加相关插件的 Caddy 程序文件已编译好，去本人 Releases 中下载即可。
14. Trojan-Go 安卓手机客户端可以去本人 Releases 中下载（最末）。
